...
class VMWareInventory(object):
    ...
    def read_settings(self):
        ...
        self.server = os.environ.get('VMWARE_HOST', config.get('vmware', 'server'))
        self.debugl('server is %s' % self.server)
        self.port = int(os.environ.get('VMWARE_PORT', config.get('vmware', 'port')))
        self.username = os.environ.get('VMWARE_USER', config.get('vmware', 'username'))
            ...
            for k, v in inventory['_meta']['hostvars'].items():
                for cat, tags_val in v['tags'].items():
                    for tag_val in tags_val:
                        tag = "{0}{1}_{2}".format(tag_prefix, cat, tag_val)
